...
Model.prototype.update = function(doc, callback) {
    ...
    // @id must exist
    if (!doc['@id']) {
        return callback(new Error('Jsonld document must have a valid @id.'));
    }
 ...
 * @param {Function} callback The callback function.
 */
Model.prototype.remove = function(query, context, callback) {
    var self = this;
    ...
    function doRemove(startNode) {
        _getAllTriples(self._client.graph, startNode, true, function (err, triples) {
    ...
    g.V(iri).Tag('subject').Out(null, 'predicate').Tag('object').All(function (err, triples) {
        ...
        async.each(triples, function (triple, cb) {
            ...
            if (utils.N3Util.isBlank(triple.object)) {
                _getAllTriples(triple.object, function (err, result) {
